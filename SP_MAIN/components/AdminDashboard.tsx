...
import { ProjectStatus, Homeowner, Installer, Admin, UserRole, User, UserStatus, RoofType, BlogPost, FinancialRecord, FinancialRecordStatus, ProjectWithDetails, EquipmentBrand, PanelModel, InverterModel, BatteryModel, HistoryLog } from '../types';
...
type AdminTab = 'projects' | 'users' | 'blog' | 'settings' | 'finance' | 'reports';
...
const AdminDashboard: React.FC<AdminDashboardProps> = (props) => {
    const { 
        ...
        onEditUser, onAddAdmin, onRoofTypeChange, onBlogPostChange, onViewChat,
    ...
    const handleSavePost = (postData: Omit<BlogPost, 'id' | 'authorId' | 'createdAt'>) => {
        let updatedPosts;
        if (editingPost === 'new') {
            ...
            {activeTab === 'blog' && (
                <Card>
                    ...
                    <div className="space-y-4">
                        {blogPosts.map(post => (
                            <div key={post.id} className="p-4 bg-slate-900/50 rounded-lg flex justify-between items-center">
                                <span>{post.title.en} / {post.title.ro}</span>
